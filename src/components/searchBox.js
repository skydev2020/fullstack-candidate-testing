...
const SearchBox = (props) => {
    ...
    const handleSearch = (word) => {
        ...
        for(let j of jobs_back) {
            ...
            for(let item of j.items) {
                if(item.job_title.toLowerCase().indexOf(word) !== -1 || item.description.toLowerCase().indexOf(word) !== -1 || item.name.toLowerCase().indexOf(word) !== -1) { 
    ...
    <div className="inputSearchContainer">
        <input type="search" id="searchBox" className="inputSearch" placeholder="Search for any job, title, keywords or company" onChange={(e) => handleSearch(e.target.value)}/>
    </div>
    );
};
