...
import closebtn from './ui_img/closebtn.svg';
import './modal.css';
    ...
    return (
        <div className="whitebox-content">
            ...
            <ul className="whitebox-data">
                {list.map((l) => (<li key={l.key} onClick={() => action(l.key)}>{l.key}<span className="item-count">{l.doc_count}</span></li>))}
                {list.length >= maxitemslist ? (<li className="list-showmore" onClick={openModal}>Show more</li>) : null} 
            </ul>
            <Modal
            ...
            >
                <div className="modalWhiteBox">
                    ...
                    <div className="row modalBody">
                        {modaldata.map((data) => (
                            <div className="col-sm-3 modalDataItem" key={data.key} onClick={() => actionModal(data.key)}>
